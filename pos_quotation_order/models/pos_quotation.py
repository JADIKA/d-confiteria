...
class PosQuotation(models.Model):
    ...
    amount_total = fields.Monetary(string='Total', store=True, readonly=True, compute='_compute_amount_all', tracking=4)
...
class PosQuotationLine(models.Model):
    ...
    price_subtotal = fields.Monetary(compute='_compute_amount_line_all', string='Subtotal', readonly=True, store=True)
    price_subtotal_incl = fields.Monetary(compute='_compute_amount_line_all', string='Total', readonly=True, store=True)
    
    order_id = fields.Many2one('pos.quotation', string='Ref. Reserva', ondelete='cascade')
    currency_id = fields.Many2one(related='order_id.pricelist_id.currency_id', depends=['order_id.pricelist_id.currency_id'], store=True, string='Currency', readonly=True)
